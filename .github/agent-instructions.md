...
- For all Submit/Cancel buttons, use the pre-defined classes in `src/styles/global.css` to style the buttons. Add additional tags (e.g. `flex-1`, `mt-2`) as needed manually in-line. Only deviate from this styling when the look of the button is too drastically different to justify using the pre-defined style.
